...
class FileReader(spark: SparkSession) extends IFileReader {
  def readFile(filePath: String): DataFrame = {
    ...
    val df = extension match {
      ...
      case "tsv" => spark.read.option("header","true").format("com.databricks.spark.csv").option("delimiter", "\t").load(filePath)
